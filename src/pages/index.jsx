...
export default function Home() {
  ...
  const handleStart = async (e) => {
    ...
    setLoading(true);
    try {
      // 1. Intentar buscar si el paciente ya existe por nombre
      ...
      // 3. Redirigir al chat con el ID del perfil
      router.push(`/chat?id=${patient.id}`);
    } catch (error) {
      console.error("Error al acceder:", error);
      alert("Hubo un problema al cargar tu perfil. Inténtalo de nuevo.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div style={{ 
      ...
      <div style={{ background: 'white', padding: '40px', borderRadius: '15px', boxShadow: '0 4px 15px rgba(0,0,0,0.1)', textAlign: 'center', maxWidth: '400px', width: '90%' }}>
        ...
        <form onSubmit={handleStart} style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
          ...
          <button
            ...
            style={{ 
              ...
              background: '#27ae60', color: 'white', fontWeight: 'bold', cursor: 'pointer',
